...
class Treasurer:
    """Only role with wallet visibility. Issues budget. Does not kill."""
    ...
    def tick(self) -> dict[str, Any]:
        ...
        if balance <= DEATH_FLOOR_USD:
            return {
                "state": "DEAD",
                "runway_hours": 0.0,
                "surplus_7d": round(self._net_7d, 2),
                "cycle_budget": 0.0,
                "may_replicate": False,
                "balance_usd": balance,
            }
        ...
        return {
            "state": state,
            "runway_hours": round(runway, 1),
            "surplus_7d": round(self._net_7d, 2),
            "cycle_budget": CYCLE_BUDGET[state],
    ...
    def fund_child(self, child_wallet: str, amount: float = CHILD_ENDOWMENT) -> None:
        self.chain.transfer(
            self.wallet, child_wallet, amount, memo="child endowment via treasurer"
        )
